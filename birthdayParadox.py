...
class BirthdayParadox:
    def __init__(self,group_size= 23,simulations:int=100):
        """
        ...
        if isinstance(group_size, BirthdayParadox):
            ...
            self._group_size = self._validate_group_size(group_size)        #assigning group size after validation
            self._simulations = self._validate_simulations(simulations)     #assigning simuation after validation
    ...
    def __str__(self):
        return f"BirthdayParadox with group size {self._group_size} and {self._simulations} simulations"

    def __repr__(self):
        return f"BirthdayParadox(group_size={self._group_size}, simulations={self._simulations})"
